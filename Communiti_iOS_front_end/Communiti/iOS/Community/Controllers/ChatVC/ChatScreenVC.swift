...
class ChatScreenVC: UIViewController , UITextViewDelegate , UITableViewDelegate, UITableViewDataSource, UIActionSheetDelegate{
    ...
    var otrOnlineStatus : DatabaseReference?
...
//    var heightPaging : CGFloat = 10.0
    ...
    override func viewDidLoad() {
        ...
        if(APP_Delegate().isAdmin){
            APP_Delegate().currentChatUser = "admin"
    ...
    func setUpdata(){
        ...
        if(APP_Delegate().isAdmin){
            ...
            if countryListObj != nil {
                  self.ChatView_H.constant = countryListObj?.chat_report  == "YES" ? 0 : 50
            }else{
                 self.ChatView_H.constant = (typeOfHelpReq == "MY_CHAT") ? (self.MYchatHomeDataModel.chat_report == "YES" ? 0 : 50) : (self.chatHomeDataModel.chat_report == "YES" ? 0 : 50)
            }
        ...
        if(UserDefaults.standard.value(forKey: "ADMIN_NM") == nil){
            self.navigationItem.title = "ADMIN"

        }else{
            self.navigationItem.title = UserDefaults.standard.value(forKey: "ADMIN_NM") as! String
        }
    ...
    func pullToDownRefresh() -> Void {
        ...
        if(!isLoading && isMoreData && self.arryChat.count > 0 ){
            ...
            self.showToast(message: "No more data", width: 150)
            removeRefreshLoader()
        }
    }
    ...
    func openDropDownMenu(){
        ...
        alert.addAction(UIAlertAction(title: "User Profile", style: .default) { action in
            self.openUserProfileOfUser()
        })
        ...
        if otherUser_id != getUserData()["usr_id"] as? String{
        
        let repostStatus  = self.countryListObj != nil ? self.countryListObj?.chat_report : (self.typeOfHelpReq == "MY_CHAT" ? self.MYchatHomeDataModel.chat_report : self.chatHomeDataModel.chat_report)
            
            
        if(post_Id == getUserData()["usr_id"] as? String && (self.MYchatHomeDataModel.pst_completed != "Yes" || self.MYchatHomeDataModel.pst_completed == "Expired")){
        ...
        if(repostStatus == "YES"){
            alert.addAction(UIAlertAction(title: "Already Reported User", style: .destructive, handler: { (action) in
            ...
            alert.addAction(UIAlertAction(title: "Report", style: .destructive) { action in
                ...
                reportView.otherUsrID = self.otherUser_id != nil ? self.otherUser_id : ""
                ...
                reportView.callbackReport = {(isSuccess : Bool) -> Void in
                    if(isSuccess){
                        DispatchQueue.main.async {
                            if self.countryListObj != nil {
                                self.ChatView_H.constant = self.countryListObj?.chat_report  == "YES" ? 0 : 50
                            }else{
                                self.ChatView_H.constant = (self.typeOfHelpReq == "MY_CHAT") ? (self.MYchatHomeDataModel.chat_report == "YES" ? 0 : 50) : (self.chatHomeDataModel.chat_report == "YES" ? 0 : 50)
                            }
                        }
                    }
                }
...
//========================================================
//MARK:- Btn UserProfile click action Action at DropDown
//========================================================
    
     func openUserProfileOfUser(){//Perform actions here
        ...
        userProfileVC.otherUserId =  otherUser_id
        self.navigationController?.pushViewController(userProfileVC, animated: true)
    }
    
//========================================================
//MARK:- Btn EndHelpReqAction click action Action
//========================================================
            ...
            if(snapshot.value != nil){
                ...
                if(self.arryChat.count > 0){
                    ...
                    if let index = (self.arryChat.index { (dict) -> Bool in dict[strDate] != nil }){
                        ...
                        dictObj[strDate]?.insert(chatScreenDM, at:(dictObj[strDate]?.count)!)
                        print(dictObj[strDate] ?? "")
                        self.arryChat[index] = dictObj
                    } else {
                ...
                for snap in arr{
                    ...
                    if(self.arryChat.count > 0){
                        ...
                        if let index = (self.arryChat.index { (dict) -> Bool in dict[strDate] != nil }){
                            ...
                            var dictObj = self.arryChat[index]
                            print(dictObj[strDate] ?? "")
                            dictObj[strDate]?.insert(chatScreenDM, at:(0))
                            print(dictObj[strDate] ?? "")
                            self.arryChat[index] = dictObj
...
//=====================================
    ...
    // ------------- Unread Count Of other user mess---------------
        if(APP_Delegate().isAdmin){
           ...
           cell.txtView_Mess.text = (chatscreenDM as AnyObject).message
           cell.lbl_Date.text  = dateWithComparisonDate(strDate: ((chatscreenDM as AnyObject).timestamp as? String)!)
           let messageSize = (cell.txtView_Mess.text?.boundingRect(with: CGSize(width: cell.frame.size.width , height: cell.frame.size.height), options: [.usesLineFragmentOrigin , .usesFontLeading], attributes: [NSFontAttributeName: Font.LucidaSans(fontSize: 16.0) as Any], context: nil).size)!
        
        let lbl_W = cell.txtView_Mess.intrinsicContentSize.width
        ...
        if(chatscreenDM?.sender == getUserData()["usr_id"] as? String){
            ...
            cell.txtView_Mess.textColor = UIColor.white
            ...
            let x = cell.bounds.size.width  - (10 + messageSize.width + 20 ) // 10 is leading dis of bg_VwOfMess
                ...
                if(170 < x){
                     ...
                     cell.txtView_Mess.textAlignment = NSTextAlignment.center

                }else{
                      cell.bgView_W.constant = 170
                      cell.bg_VwOfMess_T.constant = 10
                      cell.txtView_Mess.textAlignment = NSTextAlignment.left
                }
            ...
            cell.txtView_Mess.textColor =  UIColor(red: 135/255, green: 135/255, blue: 135/255, alpha: 1)
        ...
        cell.txtView_Mess.layoutIfNeeded()
        cell.txtView_Mess.setNeedsDisplay()
...
//    }
    ...
    @IBAction func sendMessageAction(_ sender: Any) {
       ...
       // self.arryChat.removeAll()
        ...
        DispatchQueue.main.async {
            ...
            self.checkUserOnlineStatus(txt: self.chat_txtView.text)
            self.moveToLastIndex()
            self.chat_txtView.text = ""
        }
    }

    func checkUserOnlineStatus(txt : String){
        
         otrOnlineStatus = Database.database().reference(withPath:  String(format: "/useronlinestaus/user_id_%@/status", otherUser_id))
        
        otrOnlineStatus?.observeSingleEvent(of: .value, with: { (snapshot : DataSnapshot) in
            guard snapshot.exists() else{ return  }
            
            let snapValue = snapshot.value as? [String : Any]
            
            if(snapValue?["online"] as? Int  == 0){
                self.sendMessByServer(txtMsg: txt)
            }else{
    ...
    func sendMessByServer(txtMsg : String){
        let dict : Dictionary = ["usrToken" :  getUserData()["usr_token"],
                                 "usrId" :  otherUser_id,
                                 "msg" :  txtMsg]
        
        ServerCommunicator(params: dict as Any as? Dictionary<String, Any>  , service: Service.sendFirebaseMsg , callBack: { [weak self] (response:Any ,isError: Bool) in
    ...
    func getAdminDetail(){
        ...
        ServerCommunicator(params: dict as Any as? Dictionary<String, Any>  , service: Service.getAdminDetail , callBack: { [weak self] (response:Any ,isError: Bool) in
            ...
            DispatchQueue.main.async {
                ...
                print(name)
                weakSelf.navigationItem.title = name.capitalizingFirstLetter()
                UserDefaults.standard.set(name, forKey: "ADMIN_NM")
